class HandleError:
  @staticmethod
  def check_value_if_none_or_empty(value):
    if value is None:
  ...
  def check_if_index_in_range(index, start, end):
    if index < start or index >= end:
      raise Exception("Index should be in range -1 to total length")

class Node:
  def __init__(self, value):
    HandleError.check_value_if_none_or_empty(value)
...
class SinglyLinkedList:
  ...
  def __insert_at_begin(self, value):
